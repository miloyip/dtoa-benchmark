...
#include "src/ryu/ryu_generic_128.h"
...
struct floating_decimal_128 float_to_fd128(float f) {
  ...
  return generic_binary_to_decimal(bits, FLOAT_MANTISSA_BITS,
                                   FLOAT_EXPONENT_BITS, false);
}
...
struct floating_decimal_128 double_to_fd128(double d) {
  ...
  return generic_binary_to_decimal(bits, DOUBLE_MANTISSA_BITS,
                                   DOUBLE_EXPONENT_BITS, false);
}
...
struct floating_decimal_128 long_double_to_fd128(long double d) {
  ...
  return generic_binary_to_decimal(bits, LONG_DOUBLE_MANTISSA_BITS,
                                   LONG_DOUBLE_EXPONENT_BITS, true);
}

struct floating_decimal_128
generic_binary_to_decimal(const uint128_t bits, const uint32_t mantissaBits,
  ...
  const uint32_t ieeeExponent =
      (uint32_t)((bits >> mantissaBits) & ((ONE << exponentBits) - 1u));
  ...
  if (ieeeExponent == ((1u << exponentBits) - 1u)) {
    struct floating_decimal_128 fd;
    fd.mantissa = explicitLeadingBit
                      ? ieeeMantissa & ((ONE << (mantissaBits - 1)) - 1)
                      : ieeeMantissa;
  ...
  int32_t e2;
  uint128_t m2;
  // We subtract 2 in all cases so that the bounds computation has 2 additional
  // bits.
  if (explicitLeadingBit) {
    // mantissaBits includes the explicit leading bit, so we need to correct for
    // that here.
    if (ieeeExponent == 0) {
...
static inline int copy_special_str(char *const result,
                                   const struct floating_decimal_128 fd) {
  if (fd.mantissa) {
    memcpy(result, "NaN", 3);
    return 3;
  }
...
int generic_to_chars(const struct floating_decimal_128 v, char *const result) {
  ...
  result[index] =
      '0' + (uint32_t)(output % 10); // output should be < 10 by now.
