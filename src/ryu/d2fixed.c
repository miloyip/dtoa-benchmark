...
#include "src/ryu/d2fixed_full_table.h"
#include "src/ryu/d2s_intrinsics.h"
#include "src/ryu/digit_table.h"
...
static inline uint128_t umul256(const uint128_t a, const uint64_t bHi,
                                const uint64_t bLo,
...
static inline uint128_t umul256_hi(const uint128_t a, const uint64_t bHi,
                                   const uint64_t bLo) {
...
// b.
static inline uint64_t umul256_hi128_lo64(const uint64_t aHi,
                                          const uint64_t aLo,
                                          const uint64_t bHi,
                                          const uint64_t bLo) {
  uint64_t b00Hi;
...
mulShift_mod1e9(const uint64_t m, const uint64_t *const mul, const int32_t j) {
  ...
  const uint32_t c2 =
      s1low < low2; // high1 + c1 can't overflow, so compare against low2
...
static inline void append_n_digits(const uint32_t olength, uint32_t digits,
                                   char *const result) {
#ifdef RYU_DEBUG
  printf("DIGITS=%u\n", digits);
#endif
...
int d2exp_buffered_n(double d, uint32_t precision, char *result) {
  ...
  if (e2 < 0 && availableDigits == 0) {
    ...
    for (int32_t i = MIN_BLOCK_2[idx]; i < 200; ++i) {
      ...
      digits = (p >= POW10_OFFSET_2[idx + 1])
                   ? 0
                   : mulShift_mod1e9(m2 << 8, POW10_SPLIT_2[p], j + 8);
#ifdef RYU_DEBUG
  ...
  if (lastDigit != 5) {
    ...
    if (rexp < 0) {
      ...
      trailingZeros =
          trailingZeros && multipleOfPowerOf5(m2, (uint32_t)requiredFives);
    }
    roundUp = trailingZeros ? 2 : 1;
#ifdef RYU_DEBUG
    printf("requiredTwos=%d\n", requiredTwos);
