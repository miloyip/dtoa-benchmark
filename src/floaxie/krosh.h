...
constexpr std::size_t decimal_q = std::numeric_limits<
    typename diy_fp<FloatType>::mantissa_storage_type>::digits10;

/** \brief Maximum number of necessary binary digits of fraction part. */
constexpr std::size_t fraction_binary_digits(7);

/** \brief Maximum number of decimal digits of fraction part, which can be
 * observed. */
  ...
  if ((str[0] == 'a' || str[0] == 'A') && (str[1] == 'n' || str[1] == 'N')) {
    ...
    if (*cp == '(') {
      do
        ++cp;
      while ((*cp >= '0' && *cp <= '9') ||
             (std::tolower(*cp, std::locale()) >= 'a' &&
              std::tolower(*cp, std::locale()) <= 'z') ||
             *cp == '_');
...
inline unsigned int extract_fraction_digits(const CharType *str) {
  static_assert(
      kappa <= std::numeric_limits<int>::digits10,
      "Extracting values, exceeding 'int' capacity, is not supported.");
...
template <typename FloatType, typename CharType> struct digit_parse_result {
  ...
  digit_parse_result()
      : value(), K(0), str_end(nullptr), frac(0), special(), sign(true) {}
...
inline digit_parse_result<FloatType, CharType>
parse_digits(const CharType *str) noexcept {
  ...
  while (!go_to_beach) {
    ...
    case '9':
      if (parsed_digits.empty() ||
          parsed_digits.size() + zero_substring_length <
              kappa) // we stop one digit before the maximum decimal digits to
                     // prevent overflow
      {
        ...
        if (!frac_calculated) {
          const std::size_t frac_suffix_size =
              parsed_digits.size() + zero_substring_length - kappa;
          auto tail = extract_fraction_digits<fraction_decimal_digits>(
              str + pos - frac_suffix_size);
          ret.frac = convert_numerator<fraction_decimal_digits,
                                       fraction_binary_digits>(tail);
  ...
  for (auto rit = parsed_digits.rbegin(); rit != parsed_digits.rend(); ++rit)
    ret.value +=
        (*rit) * seq_pow<typename diy_fp<FloatType>::mantissa_storage_type, 10,
                         decimal_q<FloatType>>(pow++);
 ...
 */
template <typename FloatType, typename CharType>
inline mantissa_parse_result<FloatType, CharType>
parse_mantissa(const CharType *str) {
...
inline exponent_parse_result<CharType> parse_exponent(const CharType *str) {
  ...
  if (*str != 'e' && *str != 'E') {
    ...
    ret.value = digit_parts.value *
                seq_pow<int, 10, exponent_decimal_digits>(digit_parts.K);
 ...
 */
template <typename FloatType, typename CharType>
krosh_result<FloatType, CharType> krosh(const CharType *str) {
  krosh_result<FloatType, CharType> ret;

  static_assert(
      sizeof(FloatType) <=
          sizeof(typename diy_fp<FloatType>::mantissa_storage_type),
