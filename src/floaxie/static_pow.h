...
template <unsigned int base, unsigned int pow> struct static_pow_helper {
  ...
  static const unsigned int value =
      base * static_pow_helper<base, pow - 1>::value;
};
...
constexpr unsigned long static_pow() {
 ...
 */
template <typename T, T Add, T... Seq>
struct concat_sequence<T, Add, std::integer_sequence<T, Seq...>> {
...
struct make_integer_array<std::integer_sequence<T, Ints...>> {
...
template <typename T, T base, std::size_t current_pow> struct pow_sequencer {
   ...
   */
  typedef typename concat_sequence<
      T, value,
      typename pow_sequencer<T, base, current_pow - 1>::sequence_type>::type
      sequence_type;
};
 ...
 */
template <typename T, T base, std::size_t max_pow>
constexpr T seq_pow(std::size_t pow) {
  typedef make_integer_array<
      typename pow_sequencer<T, base, max_pow>::sequence_type>
      maker;
