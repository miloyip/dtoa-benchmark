...
constexpr NumericType raised_bit(std::size_t power) {
 ...
 */
template <typename FloatType, typename NumericType = typename integer_of_size<
                                  sizeof(FloatType)>::type>
 ...
 */
template <typename NumericType>
constexpr bool nth_bit(NumericType value, std::size_t power) noexcept {
 ...
 */
template <typename NumericType>
constexpr bool highest_bit(NumericType value) noexcept {
 ...
 */
template <typename NumericType>
constexpr NumericType mask(std::size_t n) noexcept {
  static_assert(!std::is_signed<NumericType>::value,
                "Unsigned integral type is expected.");
  return n < bit_size<NumericType>() ? raised_bit<NumericType>(n) - 1
 ...
 */
template <typename NumericType>
constexpr typename std::make_unsigned<NumericType>::type
positive_part(NumericType value) noexcept {
 ...
 */
template <typename NumericType>
inline round_result round_up(NumericType last_bits,
                             std::size_t round_to_power) noexcept {
  round_result ret;

  const NumericType round_bit(raised_bit<NumericType>(round_to_power - 1));
  const NumericType check_mask(mask<NumericType>(round_to_power + 1) ^
                               round_bit);
  ret.is_accurate =
      (last_bits & mask<NumericType>(round_to_power)) != round_bit;
 ...
 */
template <typename NumericType>
constexpr NumericType constexpr_abs(NumericType value) {
