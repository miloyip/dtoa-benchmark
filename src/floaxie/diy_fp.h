 ...
 *
 * The templated type represents floating point value using two integer values,
 * one to store **mantissa** and another to hold **exponent**. Concrete types
 * are expressed as **diy_fp** specializations, with pre-defined types for
...
private:
  ...
  static_assert(
      sizeof(FloatType) == sizeof(mantissa_storage_type),
      "Float type is not compatible with its `diy_fp` representation layout.");
  ...
  static constexpr mantissa_storage_type hidden_bit() {
    return raised_bit<mantissa_storage_type>(
        std::numeric_limits<FloatType>::digits - 1);
  }
...
public:
  ...
  constexpr diy_fp(mantissa_storage_type mantissa,
                   exponent_storage_type exponent) noexcept
      : m_f(mantissa), m_e(exponent) {}
  ...
  explicit diy_fp(FloatType d) noexcept {
    constexpr auto full_mantissa_bit_size(
        std::numeric_limits<FloatType>::digits);
    constexpr auto mantissa_bit_size(full_mantissa_bit_size -
                                     1); // remember hidden bit
    constexpr mantissa_storage_type mantissa_mask(
        mask<mantissa_storage_type>(mantissa_bit_size));
    constexpr mantissa_storage_type exponent_mask(
        (~mantissa_mask) ^ msb_value<FloatType>()); // ignore sign bit
    constexpr exponent_storage_type exponent_bias(
        std::numeric_limits<FloatType>::max_exponent - 1 + mantissa_bit_size);
  ...
  downsample_result downsample() {
    ...
    constexpr auto mantissa_bit_size(full_mantissa_bit_size -
                                     1); // remember hidden bit
    constexpr mantissa_storage_type my_mantissa_size(
        bit_size<mantissa_storage_type>());
    constexpr mantissa_storage_type mantissa_mask(
        mask<mantissa_storage_type>(mantissa_bit_size));
    constexpr exponent_storage_type exponent_bias(
        std::numeric_limits<FloatType>::max_exponent - 1 + mantissa_bit_size);
    ...
    const std::size_t denorm_exp(
        positive_part(std::numeric_limits<FloatType>::min_exponent -
                      int(mantissa_bit_size) - m_e - 1));
    ...
    mantissa_storage_type parts =
        m_e + shift_amount + exponent_bias - (denorm_exp > lsb_pow);
  ...
  diy_fp &operator*=(const diy_fp &rhs) noexcept {
    ...
    const mantissa_storage_type tmp =
        (bd >> half_width) + (ad & mask_half) + (bc & mask_half) +
        raised_bit<mantissa_storage_type>(half_width - 1);
   ...
   */
  bool operator!=(const diy_fp &d) const noexcept { return !operator==(d); }
  ...
  friend std::basic_ostream<Ch, Alloc> &
  operator<<(std::basic_ostream<Ch, Alloc> &os, const diy_fp &v) {
...
private:
  ...
  void normalize_from_ieee754() noexcept {
    ...
    constexpr mantissa_storage_type my_mantissa_size(
        bit_size<mantissa_storage_type>());
    constexpr mantissa_storage_type e_diff =
        my_mantissa_size - mantissa_bit_width - 1;
