...
template <typename T, std::size_t decimal_digits, std::size_t binary_digits,
          std::size_t current_binary_digit>
struct fraction_converter<T, decimal_digits, binary_digits,
                          current_binary_digit, false> {
  ...
  static T convert(T decimal_numerator) {
    ...
    return (decision << shift_amount) |
           fraction_converter<T, decimal_digits, binary_digits,
                              current_binary_digit + 1>::convert(residue);
  }
};
...
inline T convert_numerator(T decimal_numerator) {
  return fraction_converter<T, decimal_digits, binary_digits, 1>::convert(
      decimal_numerator);
}
} // namespace floaxie
