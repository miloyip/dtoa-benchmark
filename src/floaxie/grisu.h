...
template <> struct digit_gen_select<false> {
   ...
   *
   * \tparam FloatType floating point type of `diy_fp` (`float` for single
  ...
  inline static void gen(const diy_fp<FloatType> &Mp,
                         const diy_fp<FloatType> &Mm, CharType *buffer,
                         int *len, int *K) noexcept {
    assert(Mp.exponent() <= 0);

    const diy_fp<FloatType> &delta(Mp - Mm);

    const diy_fp<FloatType> one(
        raised_bit<typename diy_fp<FloatType>::mantissa_storage_type>(
            -Mp.exponent()),
        Mp.exponent());

    half_of_mantissa_storage_type p1 = Mp.mantissa() >> -one.exponent();
    typename diy_fp<FloatType>::mantissa_storage_type p2 =
        Mp.mantissa() & (one.mantissa() - 1);
 ...
 * (http://florian.loitsch.com/publications/dtoa-pldi2010.pdf)
 */
template <int alpha, int gamma, typename FloatType, typename CharType>
inline void digit_gen(const diy_fp<FloatType> &Mp, const diy_fp<FloatType> &Mm,
                      CharType *buffer, int *len, int *K) noexcept {
  static_assert(
      static_cast<std::size_t>(constexpr_abs(alpha)) >=
              bit_size<typename diy_fp<FloatType>::mantissa_storage_type>() /
                  2 &&
          static_cast<std::size_t>(constexpr_abs(gamma)) >=
              bit_size<typename diy_fp<FloatType>::mantissa_storage_type>() / 2,
      "Current implementation supports only α and γ, which absolute values are "
      "equal or higher, "
      "than a half of integer mantissa bit size (typically 32) for performance "
      "reasons.");
 ...
 */
template <int alpha, int gamma, typename FloatType, typename CharType>
inline void grisu2(FloatType v, CharType *buffer, int *length,
                   int *K) noexcept {
  static_assert(alpha <= gamma - 3,
                "It's imposed that γ ⩾ α + 3, since otherwise it's not always "
                "possible to find a proper decimal cached power");

  std::pair<diy_fp<FloatType>, diy_fp<FloatType>> &&w(
      diy_fp<FloatType>::boundaries(v));
...
constexpr parameters grisu_parameters{
