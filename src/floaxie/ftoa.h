...
inline void ftoa(FloatType v, CharType *buffer) noexcept {
  if (std::isnan(v)) {
    ...
    *buffer = '-';
    ...
    constexpr int alpha(grisu_parameters<FloatType>.alpha),
        gamma(grisu_parameters<FloatType>.gamma);
 ...
 */
template <typename FloatType, typename CharType>
inline std::basic_string<CharType> to_basic_string(FloatType v) {
...
/** \brief 'Specialization' of `to_basic_string()` template for
 * `std::u16string`. */
